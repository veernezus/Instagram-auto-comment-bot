...
class main:
    ...
    def __init__(self) -> None:
        ...
        self.button_start = ct.CTkButton(
            master=self.root, text='Start', width=10, command=threading.Thread(target=self.button_start_event).start)
        self.button_start.pack(pady=10, padx=100)
        #self.button_start.place(rely=0.8, relx=0.43)

        # __init__

    def on_click(self, x, y, button___, pressed___):
        if self.clicked:
            return True
    ...
    def button_start_event(self):
        ...
        for i in range(self.user_number_of_comments):
            ...
            self.root.after(500)
            pyauto.press('enter')
            self.root.after((self.user_delay_of_comments * 1000) + 1200)
